...
SAVE_LOGS = BASE_DIR / "game.log"
LOGGER = logging.getLogger()

logging.basicConfig(level=logging.DEBUG, filename=SAVE_LOGS, filemode="w", format="%(asctime)s - %(levelname)s - %(message)s")
...
def character_creation(level) -> Character:
    ...
    if level == "Facile": 
        magician = Character(name = "Gandalf", _lifes = 50, _damages_min = 5, _damages_max = 15, _nb_potions = 3, _heal_min = 20, _heal_max = 30)
        foe = Character(name = "Balrog", _lifes = 20, _damages_min = 5, _damages_max = 15)
    elif level == "Normal": 
        magician = Character(name = "Gandalf", _lifes = 50, _damages_min = 5, _damages_max = 15, _nb_potions = 3, _heal_min = 20, _heal_max = 30)
        foe = Character(name = "Balrog", _lifes = 50, _damages_min = 5, _damages_max = 20)
    elif level == "Difficile": 
        magician = Character(name = "Gandalf", _lifes = 50, _damages_min = 5, _damages_max = 15, _nb_potions = 3, _heal_min = 20, _heal_max = 30)
        foe = Character(name = "Balrog", _lifes = 60, _damages_min = 5, _damages_max = 25)
    logging.DEBUG("Characters created")
    return magician, foe
...
def _display_last_results(SAVE_RESULTS):
    ...
    with open (SAVE_RESULTS, "r") as file: 
        ...
        if contents:
            print("-" * 50)
            print(f"Voici les 5 derniers résultats ")
...
def _savings_result(SAVE_RESULTS, results: str):
    ...
    if not contents:
        logging.CRITICAL("Erreur Critique : probléme avec le fichier - Fichier modifié")

    # Check  number of entries game_last_scores.json to delete oldest if it reach 10
    lenght_contents = len(contents)
    logging.DEBUG(f"Function _savings_result : {lenght_contents} entries in file")
    
    if lenght_contents > 9:
        del contents[0]
        logging.DEBUG("Function _savings_result : Number of entries reach limit - Oldest Entries deleted")
